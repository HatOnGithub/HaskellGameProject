...
insert x t@(Node bb xs tl tr bl br) | not (x `fitsIn` bb) = t
...
insertIntoQuadrant x t@(Node bb objs tl tr bl br) | x `fitsIn` bbOf tl = Node bb objs (insert x tl) tr bl br
...
getPossibleCollisionPartners obj n@(Node bb objs tl tr bl br )  | length objs  >= thresholdObjectsPerQuadrant = collapse n
